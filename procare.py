...
def main():
    ...
    view_option = st.sidebar.radio("Choose what to explore:", ["Queries", "Tables"])
    
    if view_option == "Queries":   # Sidebar for query selection
        ...
        # Rescale by updating the layout of the figure
            ...
            if not data.empty:
                ...
                fig = px.bar(
                    data,
                    x="clientname",  # Corrected column name
                    y="totalamount",  # Corrected column name
                    color="rejectionrate",  # Corrected column name
    ...
    if view_option == "Tables":
        ...
        # Map user-friendly names to actual database table names
        if selected_table == "Users":
            table_name = "users"
        elif selected_table == "Agent":
            table_name = "agent"
            ...
            table_name = "MedicalService"
        elif selected_table == "Sell":
            table_name = "sell"
        ...
        # Check if a valid table_name was determined
        if table_name:
            # Fetch and display data from the selected table
            data = fetch_table_data(table_name, engine)
            if not data.empty:
            ...
            st.error("Invalid table selected. Please try again.")
